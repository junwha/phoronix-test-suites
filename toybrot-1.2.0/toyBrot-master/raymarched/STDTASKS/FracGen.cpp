...
static size_t oversubscriptionFactor = 4;
                 ...
                 size_t h0, size_t heightStep,  size_t idx)
{
    for(size_t h = h0; h < h0+heightStep; h++)
    {
        ...
        for(size_t w = 0 + idx; w < cam.ScreenWidth(); w+= numTasks )
        {
...
bool FracGen::Generate()
{
    ...
    *  - using either std::function or std::bind (see the estimatorFunction type)
    *
    *   Lambda has you not worrying about type and is also the
    *   most sanitary conversion. Nothing in the function itself changes
    */
    auto genFrac = [](FracPtr data,
                      const Camera<tbFPType>& cam,
                      const Parameters<tbFPType>& params,
                      const estimatorFunction& f,
                      size_t h, size_t heightStep, size_t idx)
                      { return traceRegion(data, cam, params, f, h, heightStep, idx); };

    std::vector< std::future<bool> > tasks(numTasks);
    for(size_t i = 0; i < numTasks; i++)
    {
        tasks[i] = std::async(std::launch::async, genFrac, outBuffer, *cam, *parameters, box, lastHeight, heightStep, i);
    }
    ...
    , lastHeight{0}
{
    ...
    if(!bench || !once )
    {
      ...
      std::cout << "We're going to spawn " << std::thread::hardware_concurrency() * oversubscriptionFactor << " tasks" << std::endl;
    }
}
