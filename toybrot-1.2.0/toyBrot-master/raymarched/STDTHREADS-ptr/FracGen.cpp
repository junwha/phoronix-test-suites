...
using  estimatorFunction = std::function<tbFPType(Vec3<tbFPType>, const ParamPtr)>;
...
tbFPType boxDist(const Vec3<tbFPType>& p, const ParamPtr params)
{
    /**
...
RGBA getColour(const Vec4<tbFPType>& steps, const ParamPtr params )
{
    RGBA colour;
...
Vec4<tbFPType> trace(const Camera<tbFPType>& cam, const ParamPtr params, size_t x, size_t y, const estimatorFunction& f)
{
    /**
