...
{
    uint32_t A = h0*cam.ScreenWidth();
    uint32_t B = std::min(A + heightStep*cam.ScreenWidth(), cam.ScreenWidth()*cam.ScreenHeight());
    auto ptr = data.data();

    #pragma omp target teams map(to: ptr[A:B])
    #pragma omp distribute parallel for
    for(size_t i = A; i < B; i++)
    {
        ptr[i] = FracGen::getColour(trace(cam, i%cam.ScreenWidth(), i/cam.ScreenWidth()));
    }
    return false;
}
