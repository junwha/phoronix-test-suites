...
#include "FracGen.hpp"
    ...
    static std::string flavourName = "Vulkan-db";
    static std::string fileName    = "vulkan-db.c";
#else
    static std::string flavourName = "Vulkan";
    static std::string fileName    = "vulkan.c";
#endif
...
static shaderSource source = shaderSource::GLSL;
...
int runProgram(bool benching) noexcept
{
    ...
    FracGen gpuFrac(benching, camera, params, source, requestDevice);
...
void printUsage()
{
    std::vector<std::string> help
    {
        ...
        "Available compute shader sources:",
        "    <default>",
        "        GLSL compute shader",
        "    -s",
        "        HLSL compute shader",
        "    -c",
        "        OpenCL compute shader (from OpenCL project, requires clspv)",
        "",
...
int main (int argc, char** argv) noexcept
{
    ...
    enum class setting{NONE, ITERATIONS, JOBS, DIMENSIONS, HEADLESS, OUTPUT, EXPORT, HLSL, OPENCL, LIST, DEVICE};
    uint8_t dimSet = 0;
        ...
        for(int a = 1; a < argc; a++)
        {
            ...
            switch(op)
            {
                ...
                case setting::LIST:
                    ...
                    return 0;
                case setting::DEVICE:
                    requestDevice = static_cast<int>(n);
                    op = setting::NONE;
                    break;
                default:
                    break;
