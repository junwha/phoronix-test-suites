...
#include "FracGen.hpp"
    ...
    static std::string flavourName = "STD::THREADS-WASM-db";
    static std::string fileName    = "std_threads-wasm-db.c";
#else
    static std::string flavourName = "STD::THREADS-WASM";
    static std::string fileName    = "std_threads-wasm.c";
#endif
...
void initPNG(void* frac)
{
    if (frac == nullptr)
    {
        std::cout << "Cannot initPNG write, cpuFrac is null!" << std::endl;
        return;
    }
    ...
    exportWriter->setFractal(reinterpret_cast<FracGen*>(frac)->getBuffer());
    exportWriter->Init();
}
/******************************************************************************
 *
...
static constexpr const tbFPType hueFactor   =  20.0;
static constexpr const int32_t  hueOffset   =  155;
static constexpr const tbFPType valueFactor =  22;
static constexpr const tbFPType valueRange  =  0.8;
static constexpr const tbFPType valueClamp  =  1.0;
static constexpr const tbFPType satValue    =  0.5;
static constexpr const tbFPType bgRed       =  1;
static constexpr const tbFPType bgGreen     =  1;
static constexpr const tbFPType bgBlue      =  1;
static constexpr const tbFPType bgAlpha     =  1;
...
void mainLoop(void* args)
{
    ...
    while(!*(ctx->exit))
    {
        ...
        if(*(ctx->redraw))
        {
            ...
            bool completed = ctx->cpuFrac->Generate();
...
int runProgram(bool benching) noexcept
{
    ...
    FracGen cpuFrac(true, camera, params, threadFactor);

    #ifdef TOYBROT_ENABLE_PNG
    if(pngExport)
    {
        emscripten_push_main_loop_blocker(&initPNG, &cpuFrac);
    }
    #endif
    ...
    if(mainWindow != nullptr)
    {
        ...
        mainWindow->setFractal(cpuFrac.getBuffer());
            ...
            mainLoop(&ctx);
        #else
           std::cout << "Generating fractal (this will take a while and be taxing on your CPU!)" << std::endl;
           fflush(stdout);
