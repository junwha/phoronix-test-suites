...
RGBA FracGen::getColour(const RGBA& steps) const
{
    RGBA colour;
...
Vec4<tbFPType> FracGen::trace(const Camera<tbFPType>& cam, size_t x, size_t y, const estimatorFunction& f) const
{
    /**
...
void FracGen::traceRegion(FracPtr data,
                 const Camera<tbFPType>& cam,
                 const estimatorFunction& f,
                 size_t h0, size_t heightStep,  size_t idx, std::vector<bool>& results) const
{
...
bool FracGen::Generate()
{
    ...
    estimatorFunction box(&FracGen::boxDist);
    ...
    for(size_t i = 0; i < numThreads; i++)
    {
        threadPool[i] = std::thread([this, &estimator, heightStep, h = lastHeight, idx = i, &results]()
                                    {this->traceRegion(this->outBuffer, *(this->cam), estimator, h, heightStep, idx, results);});
    ...
    , lastHeight{0}
{
    ...
    numThreads = std::thread::hardware_concurrency()*oversubscriptionFactor;
