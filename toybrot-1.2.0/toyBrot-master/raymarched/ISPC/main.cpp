...
static std::string flavourName = "";

#if TOYBROT_USE_DOUBLES
    static std::string fileName    = "ispc-db.c";
#else
    static std::string fileName    = "ispc.c";
#endif
...
static LoopMode mode = LoopMode::TILED;
...
static constexpr const tbFPType hueFactor   = -30.0;
static constexpr const int32_t  hueOffset   =  295;
...
static constexpr const tbFPType satValue    =  1.0;
static constexpr const tbFPType bgRed       =  1.0;
static constexpr const tbFPType bgGreen     =  1.0;
static constexpr const tbFPType bgBlue      =  1.0;
...
int runProgram(bool benching) noexcept
{
    ...
    FracGen cpuFrac(forceHeadless?true:benching, camera, params, mode, threadFactor);
...
void printUsage()
{
    std::vector<std::string> help
    {
        ...
        "Available options",
        "    -e ",
        "        Use ISPC's regular foreach loop (default: foreach_tiled)",
        "    -s ",
        "        Iterate in a strided manner within ISPC's loop",
        "    -i X",
...
int main (int argc, char** argv) noexcept
{
    ...
    flavourName = FracGen::getFlavour();
