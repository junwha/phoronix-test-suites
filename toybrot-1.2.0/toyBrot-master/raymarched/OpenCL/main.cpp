...
#include "FracGen.hpp"
    ...
    static std::string flavourName = "OpenCL-db";
    static std::string fileName    = "OpenCL-db.c";
#else
    static std::string flavourName = "OpenCL";
    static std::string fileName    = "OpenCL.c";
#endif

static auto genTime (std::chrono::high_resolution_clock::now());
static uint32_t windowWidth = 0;
static uint32_t windowHeight = 0;
static bool requestCPU = false;
static std::string requestVendor = "";
static int requestPlatform = -1;
static int requestDevice = -1;
...
int runProgram(bool benching) noexcept
{
    ...
    FracGen gpuFrac(benching, camera, params, requestPlatform, requestDevice, requestCPU, requestVendor);
...
void printUsage()
{
    std::vector<std::string> help
    {
        ...
        "    -l",
        "        Print a list of OpenCL devices detected",
        "    -v X Y",
        "        Use OpenCL Device Y from platform X (ignores options below)",
        "    -c",
        "        Ask for a CPU OpenCL device (first found)",
        "    -a",
        "        Ask for an AMD device (first found)",
        "    -n",
        "        Ask for an nVidia device (first found)",
        "    -t",
        "        Ask for an intel device (first found)"
#ifdef TOYBROT_ENABLE_PNG
       ,"    -g",
        "        Disable gui"
#endif
...
int main (int argc, char** argv) noexcept
{
    ...
    enum class setting{NONE, ITERATIONS, DIMENSIONS, HEADLESS, OUTPUT, EXPORT,CPU, INTEL, NVIDIA, AMD, LIST, DEVICE};
    uint8_t dimSet = 0;
    uint8_t devSet = 0;
        ...
        for(int a = 1; a < argc; a++)
        {
            ...
            switch(op)
            {
                ...
                case setting::OUTPUT:
                    ...
                    if(outlog.fail())
                    {
                        std::cout << "Could not open file " << argv[a] << " for output" << std::endl;
                        return 1;
                    }
                    op = setting::NONE;
                    break;
                case setting::DIMENSIONS:
                    ...
                    if(dimSet == 0)
                    {
                        //This here to iterate to next token
                        dimSet = 1;
                        break;
                    }
                    else
                ...
                case setting::INTEL:
                    ...
                    break;
                case setting::LIST:
                    FracGen::listOpenCLDevices();
                    return 0;
                    break;
                case setting::DEVICE:
                    /*
                     * I just copied over the mess from above
                     */
                    if(devSet == 0)
                    {
                        //This here to iterate to next token
                    ...
                    else
                    {
                        if(devSet == 1)
                        {
                            requestPlatform = static_cast<size_t>(n);
                            devSet = 2;
                            break;
                        }
                        else
                        {
                            if(devSet == 2)
                            {
                                requestDevice = static_cast<size_t>(n);
                                devSet = 0;
                                op = setting::NONE;
                                break;
                            }
    ...
    if(dimSet != 0)
    {
        std::cout << "Height and Width must be either both set, or none" << std::endl;
        return 1;
    }
