     ...
     exitNow{exit}
{
    ...
    mainwindow = SDL_CreateWindow("std::threads Toybrot - Use Mouse1 to zoom in and Mouse2 to zoom out",
    ...
    texture = SDL_CreateTexture(    render,
                                    ...
                                    width, height);
    SDL_SetWindowTitle(mainwindow, "std::threads Toybrot - Use Mouse1 to zoom in and Mouse2 to zoom out");
    ...
    for(int i = 0; i < frac->h; i++)
    {
        for(int j = 0; j< frac->w; j++)
        {
           ...
           auto p = reinterpret_cast<uint32_t*>(pix) +
                    (i * highlight->w)
                    + j;
            *p = SDL_MapRGB(frac->format, 127u, 127u, 255u);
        }
    }
