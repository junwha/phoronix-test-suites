 ...
 *
 *  $Id: parse.c,v 1.91 2021/03/06 20:49:39 johns Exp $
 */
...
static errcode GetShaderMode(parsehandle * ph, SceneHandle scene) {
  ...
  while (rc == PARSENOERR) {
    ...
    if (!stringcmp(data, "END_SHADER_MODE")) {
      ...
      float aodirect;
      apicolor aoambient;
      float maxdist = RT_AO_MAXDIST_UNLIMITED;

      /* read optional maxdist parameter */
      fscanf(ph->ifp, "%s", tmp);
      if (!stringcmp(tmp, "MAXDIST")) {
        fscanf(ph->ifp, "%f", &maxdist);

        rc |= GetString(ph, "AMBIENT_COLOR");
        fscanf(ph->ifp, "%f %f %f", &aoambient.r, &aoambient.g, &aoambient.b);
      } else if (!stringcmp(tmp, "AMBIENT_COLOR")) {
        fscanf(ph->ifp, "%f %f %f", &aoambient.r, &aoambient.g, &aoambient.b);
      }
      ...
      rt_ambient_occlusion(scene, aosamples, maxdist, aoambient);
    } else {
...
static errcode GetCamera(parsehandle * ph, SceneHandle scene) {
  ...
  apiflt zoom, aspectratio, eyesep;
  ...
  if (stringcmp(data, "PROJECTION") == 0) {
    fscanf(ph->ifp, "%s", data);
    if (stringcmp(data, "EQUIRECTANGULAR") == 0) {
      rt_camera_projection(scene, RT_PROJECTION_EQUIRECTANGULAR);
    } else if (stringcmp(data, "FISHEYE") == 0) {
    ...
    } else if (stringcmp(data, "STEREO_EQUIRECTANGULAR") == 0) {
      rt_camera_projection(scene, RT_PROJECTION_STEREO_EQUIRECTANGULAR);
      rc |= GetString(ph, "EYESEPARATION");
      fscanf(ph->ifp, "%f", &a);
      eyesep = a; 
    }
  ...
  /* set stereoscopic eye separation if specified */
  if (eyesep > 0.0) {
    rt_camera_eye_separation(scene, eyesep);
  }
...
void * GetTexBody(parsehandle * ph, SceneHandle scene, int modeflag) {
  ...
  /* default texture properties if not overridden */
  ...
  if (!stringcmp(tmp, "PHONG")) {
    ...
    /* if not overridden, phong type is set to plastic by default */
...
static errcode GetSkyLight(parsehandle * ph, SceneHandle scene) {
  ...
  } else if (!stringcmp(tmp, "COLOR")) {
    fscanf(ph->ifp, "%f %f %f", &ambcol.r, &ambcol.g, &ambcol.b);
  }

  rt_ambient_occlusion(scene, numsamples, maxdist, ambcol);
