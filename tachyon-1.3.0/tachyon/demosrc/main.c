 ...
 *
 *  $Id: main.c,v 1.118 2021/12/11 05:41:19 johns Exp $
 */
...
static int fly_scene(argoptions opt, SceneHandle scene, int node) {
  ...
  rt_accumulation_mode(scene, RT_ACCUMULATE_ON);
#endif
  ...
  while (!done) {
    if (frameno != 0) {
      ...
      fps = 1.0f / (float) (rt_timer_time(fpstimer) + 0.00001f);
    } else {
      fps = 0.0f;
    }

    /* compute exponential moving average for exp(-1/10) */
    fpsexpave = (fpsexpave * 0.90f) + (fps * 0.10f);

    rt_timer_start(fpstimer);
    if (node == 0) {
      printf("\rRendering Frame: %9d   %.1f FPS       ", frameno, fpsexpave);
      fflush(stdout);
    } 
  ...
  if (node == 0) {
    ...
    printf("Animation Time: %10.4f seconds  (Averaged %.1f FPS)\n", 
...
static int animate_scene(argoptions opt, SceneHandle scene, int node) {
  ...
  if ((camfp = fopen(opt.camfilename, "r")) != NULL) {
    ...
    while (!feof(camfp) && !done) {
      ...
      if (opt.nosave == 1) {
        if (node == 0) {
          printf("\rRendering Frame: %9d   %.1f FPS       ", frameno, fps);
          fflush(stdout);
        } 
      } else {
        sprintf(outfilename, opt.outfilename, frameno);
        if (node == 0) {
          printf("\rRendering Frame to %s   (%.1f FPS)     ", outfilename, fps);
          fflush(stdout);
        }
      }
...
int main(int argc, char **argv) {
  ...
  if (node == 0) {
    ...
    sprintf(strbuf, "Copyright 1994-2021,    John E. Stone <john.stone@gmail.com> ");
    my_ui_message(0, strbuf);
    sprintf(strbuf, "------------------------------------------------------------ "); 
    my_ui_message(0, strbuf);
  }
