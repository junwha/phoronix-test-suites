 ...
 *
 * $Id: getargs.c,v 1.59 2021/03/06 20:49:39 johns Exp $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void printusage(char **argv) {
  ...
  printf("                      compensate for ambient occlusion factor.\n");
  printf("  -skylight_maxdist xxx AO maximum occlusion distance cutoff.\n");
...
static void initoptions(argoptions * opt) {
  ...
  opt->skylight_maxdist = RT_AO_MAXDIST_UNLIMITED;
...
int postsceneoptions(argoptions * opt, SceneHandle scene) {
  ...
  if (opt->auto_skylight > 0.0) {
    ...
    rt_ambient_occlusion(scene, opt->skylight_samples, opt->skylight_maxdist, col);
...
static int getparm(int argc, char **argv, int num, argoptions * opt, int node) {
  ...
  if (!strcmp(argv[num], "-skylight_maxdist")) {
    sscanf(argv[num + 1], "%f", &opt->skylight_maxdist);
    return 2;
  }
